...
module.exports.findCommitsWithAssociatedPullRequests = async ({
  ...
  if (lastRelease) {
    ...
    //    paths.forEach((path, idx) => {
    //      lastPathTrees[path] = 0
    //    })
  }

  const containsChangedTrees = (commit) => {
    //    Boolean(
    //      Object.entries(lastPathTrees).find(([path, count]) => {
    //        log({ context, message: `Found ${path}: ${count}` })
    //        return count > 0
    //      })
    //    )
  ...
  log({
    context,
    message: `Filtered: paths.length: ${paths.length}, ${allCommits.length}`,
  })
