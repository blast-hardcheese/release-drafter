...
module.exports.findCommitsWithAssociatedPullRequests = async ({
  ...
  if (lastRelease) {
    ...
    try {
      ...
      let [cs, lastCommit] = _.partition(
        _.get(data, [...dataPath, 'nodes']),
        (commit) => commit.committedDate != lastRelease.created_at
      )
      allCommits = cs
      lastCommit.forEach((commit) =>
        paths.forEach((_, idx) => {
          let { oid, path } = commit[`path${idx}`]
          lastPathTrees[`path${idx}`] = `${path} ${oid}`
        })
      )
    } catch (e) {
      log(`Exception during GraphQL, suppressing: ${e}`)
      allCommits = []
    }
  } else {
    ...
    paths.forEach((path, idx) => {
      lastPathTrees[`path${idx}`] = `${path} 0`
    })
  }

  const containsChangedTrees = (commit) =>
    Boolean(
      Object.entries(lastPathTrees).find(([key, value]) => {
        let { oid, path } = commit[key]
        return value !== `${path} ${oid}`
      })
    )
