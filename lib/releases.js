...
module.exports.findReleases = async ({
  ...
  log({ context, message: `  ${paths.length} paths` })
  const commitishFilteredReleases = filterByCommitish
    ? releases.filter((r) => ref.match(`/${r.target_commitish}$`))
    : releases
  log({
    context,
    message: `  ${commitishFilteredReleases.length} commitishFilteredReleases`,
  })
  const filteredReleases = tagPrefix
    ? commitishFilteredReleases.filter((r) => {
        log({ context, message: `${r.tag_name} starts with ${tagPrefix}` })
        return r.tag_name.startsWith(tagPrefix)
      })
    : commitishFilteredReleases
  log({ context, message: `  ${filteredReleases.length} filteredReleases` })
  const sortedPublishedReleases = sortReleases(
    filteredReleases.filter((r) => !r.draft)
  )
  log({
    context,
    message: `  sortedPublishedReleases: ${sortedPublishedReleases.length}`,
  })
  const draftRelease = filteredReleases.find((r) => r.draft)
  log({ context, message: `  draft release ${JSON.stringify(draftRelease)}` })
  const lastRelease =
...
module.exports.generateReleaseInfo = ({
  ...
  log({ context, message: `HERE0: ${name}` })
  if (name === undefined) {
    name = versionInfo
  ...
  log({
    context,
    message: `HERE: ${name} ${config['name-template']} ${JSON.stringify(
      versionInfo
    )}`,
  })
  ...
  log({
    context,
    message: `name: ${name}, tag: ${tag}, body: ..., commitish: ${commitish}, prerelease: ${isPreRelease}, draft: ${shouldDraft}`,
  })
